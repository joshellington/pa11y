...
async function runPa11yTest(url, options, state) {
	...
	const response = await gotoUrl(url, options, state);
...
async function gotoUrl(url, options, state) {
	...
	if (!options.ignoreUrl) {
		return await state.page.goto(url, {
...
async function saveScreenCapture(options, state) {
	...
	if (options.screenCapture) {
		options.log.info(
			`Capturing screen, saving with ${options.screenCapture}`
		);
		try {
			await state.page.screenshot({
				path: options.screenCapture.path,
				type: options.screenCapture.type,
				quality: options.screenCapture.quality,
				fullPage: options.screenCapture.fullPage
			});
		} catch (error) {
